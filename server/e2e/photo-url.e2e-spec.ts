...
describe('PhotoUrl Controller', () => {
    ...
    it('/GET all photo-urls ', async () => {
        const getEntities: PhotoUrlDTO[] = (await request(app.getHttpServer()).get('/api/photo-urls').expect(200)).body;
    ...
    it('/POST create photo-urls', async () => {
        const createdEntity: PhotoUrlDTO = (
            await request(app.getHttpServer()).post('/api/photo-urls').send(entityMock).expect(201)
        ).body;
    ...
    it('/PUT update photo-urls', async () => {
        const updatedEntity: PhotoUrlDTO = (
            await request(app.getHttpServer()).put('/api/photo-urls').send(entityMock).expect(201)
        ).body;
